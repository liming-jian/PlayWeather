...
fun AirQuality(airNowBean: AirNowBean.NowBean?) {
    val aqiText = airNowBean?.aqi?.takeIf { it.isNotBlank() } ?: "--"
    val aqiValue = airNowBean?.aqi?.toIntOrNull() ?: 0
    val categoryText = airNowBean?.category?.takeIf { it.isNotBlank() }
        ?: stringResource(id = R.string.air_quality_level)
    Card(
        modifier = Modifier
            .fillMaxWidth(),
